resource "proxmox_vm_qemu" "test" {
  ...
  target_node = "pve-01" # <-- Change to the name of your Proxmox node (if you have multiple nodes)
  tags        = "test,your-tag-2"
  vmid        = "803"
  ...
  # -- Cloud Init Settings
  ipconfig0  = "ip=192.168.20.82/24,gw=192.168.20.1" # <-- Change to your desired IP configuration
  nameserver = "192.168.20.1"                        # <-- Change to your desired DNS server
  ciuser     = var.username
  sshkeys    = var.ssh_key # <-- (Optional) Change to your public SSH key
}
